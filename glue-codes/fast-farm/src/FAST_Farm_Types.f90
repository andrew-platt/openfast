...
IMPLICIT NONE
    ...
    INTEGER(IntKi), PUBLIC, PARAMETER  :: ModuleFF_LidSim = 6      ! LidarSim module for lidar [-]
! =========  Farm_ParameterType  =======
...
! =======================
! =========  LidSim_Data  =======
  TYPE, PUBLIC :: LidSim_Data
    TYPE(LidarSim_ContinuousStateType)  :: x      !< Continuous states [-]
    TYPE(LidarSim_DiscreteStateType)  :: xd      !< Discrete states [-]
...
! =========  All_FastFarm_Data  =======
  TYPE, PUBLIC :: All_FastFarm_Data
    ...
    TYPE(LidSim_Data) , DIMENSION(:), ALLOCATABLE  :: LidSim      !< Data for LidarSim module [-]
...
ENDIF
    ...
    DstParamData%NumLidar = SrcParamData%NumLidar
IF (ALLOCATED(SrcParamData%Ldr_Position)) THEN
  i1_l = LBOUND(SrcParamData%Ldr_Position,1)
  i1_u = UBOUND(SrcParamData%Ldr_Position,1)
  i2_l = LBOUND(SrcParamData%Ldr_Position,2)
  i2_u = UBOUND(SrcParamData%Ldr_Position,2)
  IF (.NOT. ALLOCATED(DstParamData%Ldr_Position)) THEN 
    ALLOCATE(DstParamData%Ldr_Position(i1_l:i1_u,i2_l:i2_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
      CALL SetErrStat(ErrID_Fatal, 'Error allocating DstParamData%Ldr_Position.', ErrStat, ErrMsg,RoutineName)
      RETURN
    END IF
  END IF
    DstParamData%Ldr_Position = SrcParamData%Ldr_Position
ENDIF
IF (ALLOCATED(SrcParamData%Ldr_InFile)) THEN
  i1_l = LBOUND(SrcParamData%Ldr_InFile,1)
  i1_u = UBOUND(SrcParamData%Ldr_InFile,1)
  IF (.NOT. ALLOCATED(DstParamData%Ldr_InFile)) THEN 
    ALLOCATE(DstParamData%Ldr_InFile(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
      CALL SetErrStat(ErrID_Fatal, 'Error allocating DstParamData%Ldr_InFile.', ErrStat, ErrMsg,RoutineName)
      RETURN
    END IF
  END IF
...
ENDIF
IF (ALLOCATED(ParamData%Ldr_Position)) THEN
  DEALLOCATE(ParamData%Ldr_Position)
ENDIF
...
ENDDO
 ...
 ! buffers to store subtypes, if any
  ...
  END IF
      ...
      Int_BufSz  = Int_BufSz  + 1  ! NumLidar
  Int_BufSz   = Int_BufSz   + 1     ! Ldr_Position allocated yes/no
  IF ( ALLOCATED(InData%Ldr_Position) ) THEN
    Int_BufSz   = Int_BufSz   + 2*2  ! Ldr_Position upper/lower bounds for each dimension
      Re_BufSz   = Re_BufSz   + SIZE(InData%Ldr_Position)  ! Ldr_Position
  END IF
  Int_BufSz   = Int_BufSz   + 1     ! Ldr_InFile allocated yes/no
  ...
  END IF
    ...
    DO I = 1, LEN(InData%OutFileRoot)
      ...
      Int_Xferred = Int_Xferred + 1
    END DO ! I
    IntKiBuf(Int_Xferred) = InData%NumLidar
    Int_Xferred = Int_Xferred + 1
  IF ( .NOT. ALLOCATED(InData%Ldr_Position) ) THEN
    IntKiBuf( Int_Xferred ) = 0
    Int_Xferred = Int_Xferred + 1
  ELSE
    IntKiBuf( Int_Xferred ) = 1
    Int_Xferred = Int_Xferred + 1
    IntKiBuf( Int_Xferred    ) = LBOUND(InData%Ldr_Position,1)
    IntKiBuf( Int_Xferred + 1) = UBOUND(InData%Ldr_Position,1)
    Int_Xferred = Int_Xferred + 2
    IntKiBuf( Int_Xferred    ) = LBOUND(InData%Ldr_Position,2)
    IntKiBuf( Int_Xferred + 1) = UBOUND(InData%Ldr_Position,2)
    Int_Xferred = Int_Xferred + 2

      DO i2 = LBOUND(InData%Ldr_Position,2), UBOUND(InData%Ldr_Position,2)
        DO i1 = LBOUND(InData%Ldr_Position,1), UBOUND(InData%Ldr_Position,1)
          ReKiBuf(Re_Xferred) = InData%Ldr_Position(i1,i2)
          Re_Xferred = Re_Xferred + 1
        END DO
      END DO
  END IF
  IF ( .NOT. ALLOCATED(InData%Ldr_InFile) ) THEN
    IntKiBuf( Int_Xferred ) = 0
    Int_Xferred = Int_Xferred + 1
  ELSE
    IntKiBuf( Int_Xferred ) = 1
    Int_Xferred = Int_Xferred + 1
    IntKiBuf( Int_Xferred    ) = LBOUND(InData%Ldr_InFile,1)
    IntKiBuf( Int_Xferred + 1) = UBOUND(InData%Ldr_InFile,1)
    Int_Xferred = Int_Xferred + 2

      DO i1 = LBOUND(InData%Ldr_InFile,1), UBOUND(InData%Ldr_InFile,1)
        DO I = 1, LEN(InData%Ldr_InFile)
 ...
 ! buffers to store meshes, if any
  ...
  END IF
    ...
    DO I = 1, LEN(OutData%OutFileRoot)
      ...
      Int_Xferred = Int_Xferred + 1
    END DO ! I
    OutData%NumLidar = IntKiBuf(Int_Xferred)
    Int_Xferred = Int_Xferred + 1
  IF ( IntKiBuf( Int_Xferred ) == 0 ) THEN  ! Ldr_Position not allocated
    Int_Xferred = Int_Xferred + 1
  ELSE
    Int_Xferred = Int_Xferred + 1
    i1_l = IntKiBuf( Int_Xferred    )
    ...
    i2_u = IntKiBuf( Int_Xferred + 1)
    Int_Xferred = Int_Xferred + 2
    IF (ALLOCATED(OutData%Ldr_Position)) DEALLOCATE(OutData%Ldr_Position)
    ALLOCATE(OutData%Ldr_Position(i1_l:i1_u,i2_l:i2_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
       CALL SetErrStat(ErrID_Fatal, 'Error allocating OutData%Ldr_Position.', ErrStat, ErrMsg,RoutineName)
       RETURN
    END IF
      DO i2 = LBOUND(OutData%Ldr_Position,2), UBOUND(OutData%Ldr_Position,2)
        DO i1 = LBOUND(OutData%Ldr_Position,1), UBOUND(OutData%Ldr_Position,1)
          OutData%Ldr_Position(i1,i2) = ReKiBuf(Re_Xferred)
          Re_Xferred = Re_Xferred + 1
        END DO
      END DO
  END IF
  IF ( IntKiBuf( Int_Xferred ) == 0 ) THEN  ! Ldr_InFile not allocated
    Int_Xferred = Int_Xferred + 1
  ELSE
    Int_Xferred = Int_Xferred + 1
    i1_l = IntKiBuf( Int_Xferred    )
    i1_u = IntKiBuf( Int_Xferred + 1)
    Int_Xferred = Int_Xferred + 2
    IF (ALLOCATED(OutData%Ldr_InFile)) DEALLOCATE(OutData%Ldr_InFile)
    ALLOCATE(OutData%Ldr_InFile(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
       CALL SetErrStat(ErrID_Fatal, 'Error allocating OutData%Ldr_InFile.', ErrStat, ErrMsg,RoutineName)
       RETURN
    END IF
      DO i1 = LBOUND(OutData%Ldr_InFile,1), UBOUND(OutData%Ldr_InFile,1)
        DO I = 1, LEN(OutData%Ldr_InFile)
...
ENDIF
 ...
 SUBROUTINE Farm_CopyLidSim_Data( SrcLidSim_DataData, DstLidSim_DataData, CtrlCode, ErrStat, ErrMsg )
   TYPE(LidSim_Data), INTENT(INOUT) :: SrcLidSim_DataData
   TYPE(LidSim_Data), INTENT(INOUT) :: DstLidSim_DataData
   INTEGER(IntKi),  INTENT(IN   ) :: CtrlCode
   INTEGER(IntKi),  INTENT(  OUT) :: ErrStat
   CHARACTER(*),    INTENT(  OUT) :: ErrMsg
! Local 
...
! 
   ErrStat = ErrID_None
   ErrMsg  = ""
      CALL LidarSim_CopyContState( SrcLidSim_DataData%x, DstLidSim_DataData%x, CtrlCode, ErrStat2, ErrMsg2 )
         CALL SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg,RoutineName)
         IF (ErrStat>=AbortErrLev) RETURN
      CALL LidarSim_CopyDiscState( SrcLidSim_DataData%xd, DstLidSim_DataData%xd, CtrlCode, ErrStat2, ErrMsg2 )
         CALL SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg,RoutineName)
         IF (ErrStat>=AbortErrLev) RETURN
      CALL LidarSim_CopyConstrState( SrcLidSim_DataData%z, DstLidSim_DataData%z, CtrlCode, ErrStat2, ErrMsg2 )
         CALL SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg,RoutineName)
         IF (ErrStat>=AbortErrLev) RETURN
      CALL LidarSim_CopyOtherState( SrcLidSim_DataData%OtherSt, DstLidSim_DataData%OtherSt, CtrlCode, ErrStat2, ErrMsg2 )
         CALL SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg,RoutineName)
         IF (ErrStat>=AbortErrLev) RETURN
      ...
      CALL LidarSim_CopyOutput( SrcLidSim_DataData%y, DstLidSim_DataData%y, CtrlCode, ErrStat2, ErrMsg2 )
         CALL SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg,RoutineName)
         IF (ErrStat>=AbortErrLev) RETURN
      CALL LidarSim_CopyMisc( SrcLidSim_DataData%m, DstLidSim_DataData%m, CtrlCode, ErrStat2, ErrMsg2 )
         CALL SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg,RoutineName)
         IF (ErrStat>=AbortErrLev) RETURN
    DstLidSim_DataData%IsInitialized = SrcLidSim_DataData%IsInitialized
 END SUBROUTINE Farm_CopyLidSim_Data

 SUBROUTINE Farm_DestroyLidSim_Data( LidSim_DataData, ErrStat, ErrMsg, DEALLOCATEpointers )
  TYPE(LidSim_Data), INTENT(INOUT) :: LidSim_DataData
  INTEGER(IntKi),  INTENT(  OUT) :: ErrStat
  CHARACTER(*),    INTENT(  OUT) :: ErrMsg
  LOGICAL,OPTIONAL,INTENT(IN   ) :: DEALLOCATEpointers
  ...
  INTEGER(IntKi)                 :: ErrStat2
  CHARACTER(ErrMsgLen)           :: ErrMsg2
  CHARACTER(*),    PARAMETER :: RoutineName = 'Farm_DestroyLidSim_Data'
  ...
  CALL LidarSim_DestroyContState( LidSim_DataData%x, ErrStat2, ErrMsg2, DEALLOCATEpointers_local )
     CALL SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName)
  CALL LidarSim_DestroyDiscState( LidSim_DataData%xd, ErrStat2, ErrMsg2, DEALLOCATEpointers_local )
     CALL SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName)
  CALL LidarSim_DestroyConstrState( LidSim_DataData%z, ErrStat2, ErrMsg2, DEALLOCATEpointers_local )
     CALL SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName)
  CALL LidarSim_DestroyOtherState( LidSim_DataData%OtherSt, ErrStat2, ErrMsg2, DEALLOCATEpointers_local )
     CALL SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName)
  CALL LidarSim_DestroyParam( LidSim_DataData%p, ErrStat2, ErrMsg2, DEALLOCATEpointers_local )
     CALL SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName)
  CALL LidarSim_DestroyInput( LidSim_DataData%u, ErrStat2, ErrMsg2, DEALLOCATEpointers_local )
     CALL SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName)
  CALL LidarSim_DestroyOutput( LidSim_DataData%y, ErrStat2, ErrMsg2, DEALLOCATEpointers_local )
     CALL SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName)
  CALL LidarSim_DestroyMisc( LidSim_DataData%m, ErrStat2, ErrMsg2, DEALLOCATEpointers_local )
     CALL SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName)
 END SUBROUTINE Farm_DestroyLidSim_Data

 SUBROUTINE Farm_PackLidSim_Data( ReKiBuf, DbKiBuf, IntKiBuf, Indata, ErrStat, ErrMsg, SizeOnly )
  REAL(ReKi),       ALLOCATABLE, INTENT(  OUT) :: ReKiBuf(:)
  REAL(DbKi),       ALLOCATABLE, INTENT(  OUT) :: DbKiBuf(:)
  INTEGER(IntKi),   ALLOCATABLE, INTENT(  OUT) :: IntKiBuf(:)
  TYPE(LidSim_Data),  INTENT(IN) :: InData
  INTEGER(IntKi),   INTENT(  OUT) :: ErrStat
  CHARACTER(*),     INTENT(  OUT) :: ErrMsg
  LOGICAL,OPTIONAL, INTENT(IN   ) :: SizeOnly
    ! Local variables
  ...
  INTEGER(IntKi)                 :: ErrStat2
  CHARACTER(ErrMsgLen)           :: ErrMsg2
  CHARACTER(*), PARAMETER        :: RoutineName = 'Farm_PackLidSim_Data'
 ! buffers to store subtypes, if any
  REAL(ReKi),      ALLOCATABLE   :: Re_Buf(:)
  REAL(DbKi),      ALLOCATABLE   :: Db_Buf(:)
  INTEGER(IntKi),  ALLOCATABLE   :: Int_Buf(:)
  ...
  Int_BufSz  = 0
   ! Allocate buffers for subtypes, if any (we'll get sizes from these) 
      Int_BufSz   = Int_BufSz + 3  ! x: size of buffers for each call to pack subtype
      CALL LidarSim_PackContState( Re_Buf, Db_Buf, Int_Buf, InData%x, ErrStat2, ErrMsg2, .TRUE. ) ! x 
        CALL SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName)
        IF (ErrStat >= AbortErrLev) RETURN
      ...
      END IF
      Int_BufSz   = Int_BufSz + 3  ! xd: size of buffers for each call to pack subtype
      CALL LidarSim_PackDiscState( Re_Buf, Db_Buf, Int_Buf, InData%xd, ErrStat2, ErrMsg2, .TRUE. ) ! xd 
        CALL SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName)
        IF (ErrStat >= AbortErrLev) RETURN
      ...
      END IF
      Int_BufSz   = Int_BufSz + 3  ! z: size of buffers for each call to pack subtype
      CALL LidarSim_PackConstrState( Re_Buf, Db_Buf, Int_Buf, InData%z, ErrStat2, ErrMsg2, .TRUE. ) ! z 
        CALL SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName)
        IF (ErrStat >= AbortErrLev) RETURN
      ...
      END IF
      Int_BufSz   = Int_BufSz + 3  ! OtherSt: size of buffers for each call to pack subtype
      CALL LidarSim_PackOtherState( Re_Buf, Db_Buf, Int_Buf, InData%OtherSt, ErrStat2, ErrMsg2, .TRUE. ) ! OtherSt 
        CALL SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName)
        IF (ErrStat >= AbortErrLev) RETURN
      ...
      END IF
      Int_BufSz   = Int_BufSz + 3  ! p: size of buffers for each call to pack subtype
      CALL LidarSim_PackParam( Re_Buf, Db_Buf, Int_Buf, InData%p, ErrStat2, ErrMsg2, .TRUE. ) ! p 
        CALL SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName)
        IF (ErrStat >= AbortErrLev) RETURN
      ...
      END IF
      Int_BufSz   = Int_BufSz + 3  ! m: size of buffers for each call to pack subtype
      CALL LidarSim_PackMisc( Re_Buf, Db_Buf, Int_Buf, InData%m, ErrStat2, ErrMsg2, .TRUE. ) ! m 
        CALL SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName)
        IF (ErrStat >= AbortErrLev) RETURN
  ...
  Int_Xferred = 1
      ...
      CALL LidarSim_PackContState( Re_Buf, Db_Buf, Int_Buf, InData%x, ErrStat2, ErrMsg2, OnlySize ) ! x 
        CALL SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName)
        IF (ErrStat >= AbortErrLev) RETURN
      ...
      ELSE
        IntKiBuf( Int_Xferred ) = 0; Int_Xferred = Int_Xferred + 1
      ENDIF
      CALL LidarSim_PackDiscState( Re_Buf, Db_Buf, Int_Buf, InData%xd, ErrStat2, ErrMsg2, OnlySize ) ! xd 
        CALL SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName)
        IF (ErrStat >= AbortErrLev) RETURN
      ...
      ELSE
        IntKiBuf( Int_Xferred ) = 0; Int_Xferred = Int_Xferred + 1
      ENDIF
      CALL LidarSim_PackConstrState( Re_Buf, Db_Buf, Int_Buf, InData%z, ErrStat2, ErrMsg2, OnlySize ) ! z 
        CALL SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName)
        IF (ErrStat >= AbortErrLev) RETURN
      ...
      ELSE
        IntKiBuf( Int_Xferred ) = 0; Int_Xferred = Int_Xferred + 1
      ENDIF
      CALL LidarSim_PackOtherState( Re_Buf, Db_Buf, Int_Buf, InData%OtherSt, ErrStat2, ErrMsg2, OnlySize ) ! OtherSt 
        CALL SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName)
        IF (ErrStat >= AbortErrLev) RETURN
      ...
      ELSE
        IntKiBuf( Int_Xferred ) = 0; Int_Xferred = Int_Xferred + 1
      ENDIF
      CALL LidarSim_PackParam( Re_Buf, Db_Buf, Int_Buf, InData%p, ErrStat2, ErrMsg2, OnlySize ) ! p 
        CALL SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName)
        IF (ErrStat >= AbortErrLev) RETURN
      ...
      ELSE
        IntKiBuf( Int_Xferred ) = 0; Int_Xferred = Int_Xferred + 1
      ENDIF
      CALL LidarSim_PackMisc( Re_Buf, Db_Buf, Int_Buf, InData%m, ErrStat2, ErrMsg2, OnlySize ) ! m 
        CALL SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName)
        IF (ErrStat >= AbortErrLev) RETURN
    ...
    IntKiBuf(Int_Xferred) = TRANSFER(InData%IsInitialized, IntKiBuf(1))
    Int_Xferred = Int_Xferred + 1
 END SUBROUTINE Farm_PackLidSim_Data

 SUBROUTINE Farm_UnPackLidSim_Data( ReKiBuf, DbKiBuf, IntKiBuf, Outdata, ErrStat, ErrMsg )
  REAL(ReKi),      ALLOCATABLE, INTENT(IN   ) :: ReKiBuf(:)
  REAL(DbKi),      ALLOCATABLE, INTENT(IN   ) :: DbKiBuf(:)
  INTEGER(IntKi),  ALLOCATABLE, INTENT(IN   ) :: IntKiBuf(:)
  TYPE(LidSim_Data), INTENT(INOUT) :: OutData
  INTEGER(IntKi),  INTENT(  OUT) :: ErrStat
  CHARACTER(*),    INTENT(  OUT) :: ErrMsg
    ! Local variables
  INTEGER(IntKi)                 :: Buf_size
 ...
 ! buffers to store meshes, if any
  ...
  Int_Xferred  = 1
      ...
      IF(Buf_size > 0) THEN
        ...
        Int_Xferred = Int_Xferred + Buf_size
      END IF
      CALL LidarSim_UnpackContState( Re_Buf, Db_Buf, Int_Buf, OutData%x, ErrStat2, ErrMsg2 ) ! x 
        CALL SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName)
        IF (ErrStat >= AbortErrLev) RETURN
      ...
      IF(Buf_size > 0) THEN
        ...
        Int_Xferred = Int_Xferred + Buf_size
      END IF
      CALL LidarSim_UnpackDiscState( Re_Buf, Db_Buf, Int_Buf, OutData%xd, ErrStat2, ErrMsg2 ) ! xd 
        CALL SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName)
        IF (ErrStat >= AbortErrLev) RETURN
      ...
      IF(Buf_size > 0) THEN
        ...
        Int_Xferred = Int_Xferred + Buf_size
      END IF
      CALL LidarSim_UnpackConstrState( Re_Buf, Db_Buf, Int_Buf, OutData%z, ErrStat2, ErrMsg2 ) ! z 
        CALL SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName)
        IF (ErrStat >= AbortErrLev) RETURN
      ...
      IF(Buf_size > 0) THEN
        ...
        Int_Xferred = Int_Xferred + Buf_size
      END IF
      CALL LidarSim_UnpackOtherState( Re_Buf, Db_Buf, Int_Buf, OutData%OtherSt, ErrStat2, ErrMsg2 ) ! OtherSt 
        CALL SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName)
        IF (ErrStat >= AbortErrLev) RETURN
      ...
      IF(Buf_size > 0) THEN
        ...
        Int_Xferred = Int_Xferred + Buf_size
      END IF
      CALL LidarSim_UnpackParam( Re_Buf, Db_Buf, Int_Buf, OutData%p, ErrStat2, ErrMsg2 ) ! p 
        CALL SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName)
        IF (ErrStat >= AbortErrLev) RETURN
      ...
      IF(Buf_size > 0) THEN
        ...
        Int_Xferred = Int_Xferred + Buf_size
      END IF
      CALL LidarSim_UnpackOutput( Re_Buf, Db_Buf, Int_Buf, OutData%y, ErrStat2, ErrMsg2 ) ! y 
        CALL SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName)
        IF (ErrStat >= AbortErrLev) RETURN
      ...
      IF(Buf_size > 0) THEN
        ...
        Int_Xferred = Int_Xferred + Buf_size
      END IF
      CALL LidarSim_UnpackMisc( Re_Buf, Db_Buf, Int_Buf, OutData%m, ErrStat2, ErrMsg2 ) ! m 
        CALL SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName)
        IF (ErrStat >= AbortErrLev) RETURN
    ...
    OutData%IsInitialized = TRANSFER(IntKiBuf(Int_Xferred), OutData%IsInitialized)
    Int_Xferred = Int_Xferred + 1
 END SUBROUTINE Farm_UnPackLidSim_Data
...
ENDIF
      ...
      CALL Farm_Copymd_data( SrcAll_FastFarm_DataData%MD, DstAll_FastFarm_DataData%MD, CtrlCode, ErrStat2, ErrMsg2 )
         CALL SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg,RoutineName)
         IF (ErrStat>=AbortErrLev) RETURN
IF (ALLOCATED(SrcAll_FastFarm_DataData%LidSim)) THEN
  i1_l = LBOUND(SrcAll_FastFarm_DataData%LidSim,1)
  i1_u = UBOUND(SrcAll_FastFarm_DataData%LidSim,1)
  IF (.NOT. ALLOCATED(DstAll_FastFarm_DataData%LidSim)) THEN 
    ALLOCATE(DstAll_FastFarm_DataData%LidSim(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
      CALL SetErrStat(ErrID_Fatal, 'Error allocating DstAll_FastFarm_DataData%LidSim.', ErrStat, ErrMsg,RoutineName)
      RETURN
    END IF
  END IF
    DO i1 = LBOUND(SrcAll_FastFarm_DataData%LidSim,1), UBOUND(SrcAll_FastFarm_DataData%LidSim,1)
      CALL Farm_Copylidsim_data( SrcAll_FastFarm_DataData%LidSim(i1), DstAll_FastFarm_DataData%LidSim(i1), CtrlCode, ErrStat2, ErrMsg2 )
         CALL SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg,RoutineName)
         IF (ErrStat>=AbortErrLev) RETURN
    ENDDO
ENDIF
...
ENDIF
  ...
  CALL Farm_Destroymd_data( All_FastFarm_DataData%MD, ErrStat2, ErrMsg2, DEALLOCATEpointers_local )
     CALL SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName)
IF (ALLOCATED(All_FastFarm_DataData%LidSim)) THEN
DO i1 = LBOUND(All_FastFarm_DataData%LidSim,1), UBOUND(All_FastFarm_DataData%LidSim,1)
  CALL Farm_Destroylidsim_data( All_FastFarm_DataData%LidSim(i1), ErrStat2, ErrMsg2, DEALLOCATEpointers_local )
     CALL SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName)
ENDDO
  DEALLOCATE(All_FastFarm_DataData%LidSim)
ENDIF
 ...
 ! buffers to store subtypes, if any
  ...
  END IF
      ...
      IF(ALLOCATED(Int_Buf)) THEN ! MD
         ...
         DEALLOCATE(Int_Buf)
      END IF
  Int_BufSz   = Int_BufSz   + 1     ! LidSim allocated yes/no
  IF ( ALLOCATED(InData%LidSim) ) THEN
    Int_BufSz   = Int_BufSz   + 2*1  ! LidSim upper/lower bounds for each dimension
    DO i1 = LBOUND(InData%LidSim,1), UBOUND(InData%LidSim,1)
      Int_BufSz   = Int_BufSz + 3  ! LidSim: size of buffers for each call to pack subtype
      CALL Farm_Packlidsim_data( Re_Buf, Db_Buf, Int_Buf, InData%LidSim(i1), ErrStat2, ErrMsg2, .TRUE. ) ! LidSim 
        CALL SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName)
        IF (ErrStat >= AbortErrLev) RETURN

      IF(ALLOCATED(Re_Buf)) THEN ! LidSim
         Re_BufSz  = Re_BufSz  + SIZE( Re_Buf  )
         DEALLOCATE(Re_Buf)
      END IF
      IF(ALLOCATED(Db_Buf)) THEN ! LidSim
         Db_BufSz  = Db_BufSz  + SIZE( Db_Buf  )
         DEALLOCATE(Db_Buf)
      END IF
      IF(ALLOCATED(Int_Buf)) THEN ! LidSim
         Int_BufSz = Int_BufSz + SIZE( Int_Buf )
         DEALLOCATE(Int_Buf)
      END IF
    END DO
  ...
  END IF
      ...
      ELSE
        IntKiBuf( Int_Xferred ) = 0; Int_Xferred = Int_Xferred + 1
      ENDIF
  IF ( .NOT. ALLOCATED(InData%LidSim) ) THEN
    IntKiBuf( Int_Xferred ) = 0
    Int_Xferred = Int_Xferred + 1
  ELSE
    IntKiBuf( Int_Xferred ) = 1
    Int_Xferred = Int_Xferred + 1
    IntKiBuf( Int_Xferred    ) = LBOUND(InData%LidSim,1)
    IntKiBuf( Int_Xferred + 1) = UBOUND(InData%LidSim,1)
    Int_Xferred = Int_Xferred + 2

    DO i1 = LBOUND(InData%LidSim,1), UBOUND(InData%LidSim,1)
      CALL Farm_Packlidsim_data( Re_Buf, Db_Buf, Int_Buf, InData%LidSim(i1), ErrStat2, ErrMsg2, OnlySize ) ! LidSim 
        CALL SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName)
        IF (ErrStat >= AbortErrLev) RETURN
 ...
 ! buffers to store meshes, if any
  ...
  END IF
      ...
      IF(ALLOCATED(Db_Buf )) DEALLOCATE(Db_Buf )
      IF(ALLOCATED(Int_Buf)) DEALLOCATE(Int_Buf)
  IF ( IntKiBuf( Int_Xferred ) == 0 ) THEN  ! LidSim not allocated
    Int_Xferred = Int_Xferred + 1
  ELSE
    Int_Xferred = Int_Xferred + 1
    i1_l = IntKiBuf( Int_Xferred    )
    i1_u = IntKiBuf( Int_Xferred + 1)
    Int_Xferred = Int_Xferred + 2
    IF (ALLOCATED(OutData%LidSim)) DEALLOCATE(OutData%LidSim)
    ALLOCATE(OutData%LidSim(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
       CALL SetErrStat(ErrID_Fatal, 'Error allocating OutData%LidSim.', ErrStat, ErrMsg,RoutineName)
       RETURN
    END IF
    DO i1 = LBOUND(OutData%LidSim,1), UBOUND(OutData%LidSim,1)
      Buf_size=IntKiBuf( Int_Xferred )
      Int_Xferred = Int_Xferred + 1
      IF(Buf_size > 0) THEN
        ALLOCATE(Re_Buf(Buf_size),STAT=ErrStat2)
      ...
      IF(Buf_size > 0) THEN
        ...
        Int_Xferred = Int_Xferred + Buf_size
      END IF
      CALL Farm_Unpacklidsim_data( Re_Buf, Db_Buf, Int_Buf, OutData%LidSim(i1), ErrStat2, ErrMsg2 ) ! LidSim 
        CALL SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName)
        IF (ErrStat >= AbortErrLev) RETURN
